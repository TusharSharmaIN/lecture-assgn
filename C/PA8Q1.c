 ...
 *	Problem Statement
 *
 *	In this question, you have to write code to remove the first node in a doubly linked list containing a specified number. 
 *	The code to create the linked list is already given. 
 *	The main() function calls a function removeData(head,tail,n) to remove the first node containing n from the linked list.
 *
 ...
 *	Note that you do not have to add any additional code to print the list.
 *	Once you code the removeData() function, the code will print the contents of the remaining list.
 *
 */
...
void removeData(struct DoublyLinkedList **headptr, struct DoublyLinkedList **tailptr, int num)
{
	
	struct DoublyLinkedList* ptr = *headptr;
	...
	if(ptr->data == num)
	{
        ...
        {
			struct DoublyLinkedList *temp = ptr->next;
          	*headptr = temp;
        }
      	free(ptr);     	
	}
	else
	{
		...
		struct DoublyLinkedList* prev = *headptr;
